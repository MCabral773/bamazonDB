        ...
        connection.query('SELECT stock_quantity FROM products WHERE ?', {item_id: chosenItem.item_id }, (err, res) => {
            if (res[0].stock_quantity < answer.howMany) {
                ...
                inquirer.prompt({
                    name: 'proceed',
                    type: 'confirm',
                    message: 'Would you still ike to purchase this product?'
                }).then ((answer) => {
                    if (answer.proceed) {
                        askHowMany(chosenItem.item_id);
                    } else {
                ...
                connection.query('UPDATE products SET ? WHERE ?', [
                    ...
                    console.log(chalk.blue.bold (`\n\t Order confirmed! Your total purchase was $${(chosenItem.price * chosenItem.howmany).toFixed(2)}.\n`));
                    promptNewPurchase();
                });
            }
        });
